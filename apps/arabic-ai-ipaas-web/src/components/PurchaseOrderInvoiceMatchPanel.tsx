...
export const PurchaseOrderInvoiceMatchPanel: React.FC<{documents:DocumentRecord[]; language:'ar'|'en'}>=({documents,language})=>{
 ...
 return <Card><CardHeader title={language==='ar'?'مطابقة أمر الشراء والفاتورة':'PO ↔ Invoice Matching'} subtitle={language==='ar'?'مقارنة حتمية ثم قرار بشري موثق':'Deterministic comparison followed by a governed human decision'}/><CardBody className="space-y-4 font-arabic">
  ...
  {match&&<div className="space-y-3 rounded-xl border p-4">
   <div className="flex items-center gap-3"><Badge variant={match.status==='matched'?'success':match.status==='mismatch'?'error':'warning'} size="sm">{match.status}</Badge><span className="text-sm font-bold">{language==='ar'?'درجة المطابقة الأساسية':'Core match score'}: {match.score}%</span></div>
   <div className="text-[11px] text-slate-500">{language==='ar'?'الدرجة تشمل المورد والعملة والإجمالي ومقارنات البنود؛ التفاصيل تظهر أدناه.':'The score includes supplier, currency, grand total, and line-item comparisons; details appear below.'}</div>
   ...
   {latest&&<div className="rounded-lg bg-slate-50 p-3 text-xs"><b>{language==='ar'?'آخر قرار':'Latest decision'}:</b> {latest.decision}<div className="mt-1 text-slate-600">{latest.reason}</div><div className="mt-2 break-all font-mono text-[10px] text-slate-500">{language==='ar'?'بصمة المطابقة':'Match digest'}: {latest.matchDigest}</div><div className="mt-1 text-[10px] text-slate-500">PO extraction: {latest.purchaseOrderExtractionId} · Invoice extraction: {latest.invoiceExtractionId}</div></div>}
